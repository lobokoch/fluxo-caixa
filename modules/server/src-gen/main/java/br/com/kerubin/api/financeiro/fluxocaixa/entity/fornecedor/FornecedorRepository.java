...
public interface FornecedorRepository extends JpaRepository<FornecedorEntity, java.util.UUID>, QuerydslPredicateExecutor<FornecedorEntity> {
	...
	@Query("select distinct ac.id as id, ac.nome as nome, ac.cpfCNPJ as cpfCNPJ from FornecedorEntity ac where ( upper(ac.nome) like upper(concat('%', :query, '%')) ) or ( upper(ac.cpfCNPJ) like upper(concat('%', :query, '%')) ) order by 1 asc")
